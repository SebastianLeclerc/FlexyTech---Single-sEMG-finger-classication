from src import combine_data, np, butter, lfilter
...
def normalize_data(sample: np.array):
    ...
    min_vector = np.ones(
        len(sample)) * min  # Creates a vector of only 1 with the correct length and multiplies it with the minimum value
...
def format_timestamps(time_stamps: np.array):
    ...
    '''
    time_vector = np.ones(len(time_stamps)) * time_stamps[
        0]  # Creates a vector of only 1 with the correct length and multiplies it with the first time stamp
